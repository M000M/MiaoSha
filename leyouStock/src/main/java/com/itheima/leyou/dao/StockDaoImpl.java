...
public class StockDaoImpl implements IStockDao {
    ...
    public ArrayList<Map<String, Object>> getStock(String sku_id){
        //1、创建商品查询的SQL
        String sql = "SELECT tb_sku.spu_id, tb_sku.title, tb_sku.images, tb_sku.stock, tb_sku.price, " +
                "tb_sku.indexes, tb_sku.own_spec, tb_sku.enable, tb_sku.create_time, tb_sku.update_time, " +
                "tb_spu_detail.description, tb_sku.id AS sku_id, tb_spu_detail.special_spec FROM tb_sku " +
                "INNER JOIN tb_spu_detail ON tb_spu_detail.spu_id=tb_sku.spu_id WHERE tb_sku.id = ?";
        //2、执行SQL
